...
import { addMinutes, isBefore } from "./deps.ts";
import { hmac, sha512 } from "./deps.ts";
import { pkg } from "./deps.ts";
...
app.use(async (ctx, next) => {
  if (
    Deno.env.USE_SIGNED_LINKS === "active" &&
    ctx.request.path === "/pdf"
  ) {
    const { sig, url, exp } = ctx.request.query;
    if (sig && url && exp) {
      const mac = hmac(sha512, Deno.env.HMAC_KEY, exp + "|" + url);
...
<html class="bg-base-300">
  ...
  <body>
      ...
      <pre data-prefix=">"><code> </code></pre> 
      <pre data-prefix=">"><code>Version ${pkg.version}</code></pre>      
    </div>
  </body>
</html>`;
  })
...
app.use(
  r.all("/pdf", async (ctx) => {
    // TODO: TOKEN check Deno.env.TOKEN
...
app.use(
  r.all("/_hmac", async (ctx) => {
    ...
    const mac = hmac(sha512, Deno.env.HMAC_KEY, d + "|" + url);
...
app.listen(Deno.env.PORT || 3033, "0.0.0.0", async () => {
